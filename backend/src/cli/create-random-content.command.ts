...
async function bootstrap(): Promise<void> {
  ...
  try {
    ...
    await mediaModel.insertMany([
      { userId: faker.random.arrayElement(users)._id, path: path.resolve('./uploads/burger-1.jpeg') },
      { userId: faker.random.arrayElement(users)._id, path: path.resolve('./uploads/burger-2.jpeg') },
      { userId: faker.random.arrayElement(users)._id, path: path.resolve('./uploads/burger-3.jpeg') },
      { userId: faker.random.arrayElement(users)._id, path: path.resolve('./uploads/pizza-1.jpeg') },
      { userId: faker.random.arrayElement(users)._id, path: path.resolve('./uploads/pizza-2.jpeg') },
      { userId: faker.random.arrayElement(users)._id, path: path.resolve('./uploads/pizza-3.jpeg') },
      { userId: faker.random.arrayElement(users)._id, path: path.resolve('./uploads/pizza-4.jpeg') },
    ])
    ...
    do {
      ...
      for (const user of users) {
        const quantity = faker.datatype.number({ min: 2000, max: 3000 })
